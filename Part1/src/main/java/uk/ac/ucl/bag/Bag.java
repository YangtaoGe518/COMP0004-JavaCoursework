...
public interface Bag<T extends Comparable> extends Iterable<T>
{
  ...
  /**
   * It mkaes sure that the occurrence count of all values stored in a Bag is set to on.
   */
  void removeAllCopies();

  /**
   * It get a new Bag containing all values and occurrences that in the this bag but not the argument bag
   * @param bag The bag to check duplicated elements and subtract
   * @return The new bag
   */
